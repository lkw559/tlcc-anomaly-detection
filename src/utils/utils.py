...
def create_data_loaders(train_dataset, batch_size, val_split=0.1, shuffle=True, test_dataset=None):
    ...
    if val_split == 0.0:
        ...
        train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=shuffle, generator=g)
        ...
        ########## indices 수정 ###########
        """indices = []
        indices_before = list(range(dataset_size))
        for i in range(len(indices_before)):
            if indices_before[i] % 3 == 0: ###### stride 설정 3, 5, 7
                indices.append(indices_before[i])
        print('Adopt Stride len(ind ices):', len(indices))"""
        ...
        train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, sampler=train_sampler, generator=g)
        val_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, sampler=valid_sampler, generator=g)
...
#################### robust #####################
def create_data_robust_loaders(train_robust_dataset, batch_size, val_split=0.1, shuffle=False):
    train_robust_loader, val_robust_loader  = None, None
    if val_split == 0.0:
        print(f"train_robust_size: {len(train_robust_dataset)}")
        train_loader = torch.utils.data.DataLoader(train_robust_dataset, batch_size=batch_size, shuffle=shuffle, generator=g)

    else:
        dataset_size = len(train_robust_dataset)
        indices = list(range(dataset_size))
        print('len(indices):', len(indices))
        ########## indices 수정 ###########
        indices = []
        indices_before = list(range(dataset_size))
        for i in range(len(indices_before)):
            if indices_before[i] % 100 == 0: ###### stride 설정 100
                indices.append(indices_before[i])
        print('Adopt Stride len(ind ices):', len(indices))
        #split = int(np.floor(val_split * dataset_size))
        split = int(np.floor(val_split * len(indices)))
        ...
        train_robust_sampler = SubsetRandomSampler(train_indices)
        print('train_robust_sampler:', train_robust_sampler)
        valid_robust_sampler = SubsetRandomSampler(val_indices)

        train_robust_loader = torch.utils.data.DataLoader(train_robust_dataset, batch_size=batch_size, sampler=train_robust_sampler)
        val_robust_loader = torch.utils.data.DataLoader(train_robust_dataset, batch_size=batch_size, sampler=valid_robust_sampler)

        print(f"train_robust_size: {len(train_indices)}")
        print(f"validation_robust_size: {len(val_indices)}")


    return train_robust_loader, val_robust_loader
